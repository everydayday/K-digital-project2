	...
	// @RequestParam String userLoginId, @RequestParam String userPw
		...
		public ResponseEntity<Map<String, String>> login(@RequestBody User user) {
			...
			try {
				...
				User encodeUser = userRepo.findUserPwByUserLoginId(user.getUserLoginId());	
...
//				logger.error("Exception occurred during login: {} ", e.getMessage());
		...
		// 존재하는 id인지 
		@PostMapping("/checkid")
		public ResponseEntity<Map<String, String>> checkidByPhoneNumber(@RequestBody User user){
			Map<String, String> response = new HashMap<>();
		...
		public ResponseEntity<Map<String, String>> resetPassword(@RequestBody User user){
			Map<String, String> response = new HashMap<>();
			System.out.println("user in resetPasswonrd controller :" + user.getUserLoginId());
